...
echo "     h) 老环境如遇 streamTo/runtime=subagent 报错，重新运行 install.sh 会自动补兼容补丁"
echo ""
...
echo ""
echo -e "  ${YELLOW}6. OpenMOSS 治理层说明（如需）${NC}"
echo "     OpenMOSS state 目录默认不会在安装阶段预创建。"
echo "     首次进入治理页或首次创建治理任务时，系统会按需创建："
echo "       ~/.openclaw/state/openmoss/"
echo "     如果当前还没使用治理能力，目录不存在属于正常现象。"
echo "     建议先运行 doctor.sh 做只读检查，再进入 GUI 验证。"
echo "     参考文档："
echo "       docs/openmoss/adr-002-installer-minimum-delivery-policy.md"
echo "       docs/host-install-migration.md"
echo ""
