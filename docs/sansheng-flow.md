...
### 触发条件
用户真实 mention 司礼监 Bot 下达任务（非闲聊/非简单问答）
...
4. **司礼监** 拿到内阁的优化结果后，用预先配置好的 `<@BotUserId>` mention 模板在频道内派发
...
```
用户：在 Discord 客户端真实 mention 司礼监 Bot，然后发送“帮我写一个用户登录功能”
...
内阁 → [返回优化结果]
       「【优化后 Prompt】...
        【执行计划】
        Step 1: <@兵部BotUserId> — 实现 JWT 认证后端 API
        Step 2: <@兵部BotUserId> — 实现 React 登录页面组件
        Step 3: <@都察院BotUserId> — push 后自动审查」

司礼监 → [在频道派活]
       「<@兵部BotUserId> 【任务】实现用户登录功能...」
